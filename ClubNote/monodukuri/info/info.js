...
for (let i = 1; i <= 3; i++) {
  ...
  function checkOverallStatus() {
    const allComplete = Array.from({ length: 3 }, (_, i) => {
      return document.getElementById(`btn${i + 1}`).textContent === 'ダウンロード完了';
    }).every(Boolean);
  
    // .t-info に info-alert クラスの付け外し
    document.querySelectorAll('.t-info, .c-3 .dl-menu').forEach(tInfo => {
      if (!allComplete) {
  ...
  // idに応じたラベルを設定（a〜z対応）
  const labels = ['ガイドブック', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j'];
  const label = labels[id - 1] || '';
  const dl_sizes = ['416 KB', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j'];
  ...
  block.innerHTML = `
    ...
    <span style="width: 180px; font-size: 1.2rem">${dl_size}</span>
  `;
