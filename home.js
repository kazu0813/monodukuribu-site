        ...
        class GoogleAuthExtension {
            ...
            getInfo() {
                return {
                    ...
                    color3: '#2A56C6',
                    blocks: [
                        { opcode: 'login', blockType: Scratch.BlockType.COMMAND, text: 'Open Google Login Page' },
                        { opcode: 'clearData', blockType: Scratch.BlockType.COMMAND, text: 'Clear data' },
                        { opcode: 'isLoginWindowOpen', blockType: Scratch.BlockType.BOOLEAN, text: 'Is login window open?' },
                        { opcode: 'getAccountName', blockType: Scratch.BlockType.REPORTER, text: 'Account Email' },
                        { opcode: 'getFullName', blockType: Scratch.BlockType.REPORTER, text: 'Full Name' },
                        { opcode: 'getProfilePicture', blockType: Scratch.BlockType.REPORTER, text: 'Profile Picture URL' },
                        { opcode: 'getUserId', blockType: Scratch.BlockType.REPORTER, text: 'User ID' },
                        { opcode: 'getLocale', blockType: Scratch.BlockType.REPORTER, text: "User's preferred language" },
                        { opcode: 'isEmailVerified', blockType: Scratch.BlockType.BOOLEAN, text: 'Email verified?' },
                    ],
        ...
        function accountPage() {
            const auth = new GoogleAuthExtension();
            auth.login();

            window.addEventListener('message', (event) => {
                if (event.origin === 'https://ikelene.ca') {
                    localStorage.setItem('name', event.data.fullName || '');
                    localStorage.setItem('mail', event.data.accountName || '');
                    localStorage.setItem('icon', event.data.profilePicture || 'icon-no-login.png');
                    updateUI();
                }
            });
        }
